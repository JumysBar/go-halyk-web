...
func main() {
	...
	kafkaWriter := &kafka.Writer{
		Addr: kafka.TCP("kafka-1:9092",
			"kafka-2:9092",
			"kafka-3:9092"), // тут все понятно - вы должны указать все адреса всего кластера

		Balancer: &kafka.LeastBytes{}, // реализация балансировки между партициями, по дефолту &kafka.RoundRobin{}.
		//
		RequiredAcks: kafka.RequireAll, // достаточно важное поле - нужны ли acknowledgements от kafka, при записи сообщений.
